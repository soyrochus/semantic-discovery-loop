...
- **Coordinate phases** in the order given in LOOP.md, invoking skills 01–08. Skip to
  the corrective action in `state.json.next_action` when resuming a failed verification.
...
3. Run phases 01–06 (or resume from `next_action`), validating each artefact against its
   schema in `contracts/` before moving on.
