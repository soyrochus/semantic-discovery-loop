...
The contract has since been extended per `specs/implement-extended-semantic-layer.md` (Milestones 1-2): provenance entries carry an evidence `kind` (`parsed | observed | asserted`) under the authority rule `parsed > observed > asserted`, a documentation-alignment phase (skill 10, `doc-claims-v1` tool) produces `doc-claims.json`, and the gate now measures **nine** dimensions (adding `assertion_grounding`) with a 6-mutation self-test (adding `forged-doc-claim` and `asserted-only-accepted`). The "eight" counts in the 2026-07-05 addendum describe the contract as it stood then. `examples/taskdesk-legacy-run/` was regenerated under the nine-dimension contract: all scores 10, self-test 6/6, 8 documentation claims (7 confirmed, 1 unverifiable - the latter now grounds `sem:unknown:runtime-row-semantics` as disciplined asserted evidence with a `claim_ref`).

## Addendum (2026-07-07, later): contract version 2, ten dimensions

Milestone 3 (Extension B, runtime journeys) completed the extension. `LOOP.md` is now **contract version 2**: the "no web automation" exclusion is replaced by the approval-gated, sandboxed runtime journey phase (skill 09, `contracts/journeys.schema.json`). `observed` evidence is un-reserved — it may instantiate behavioural `Flow` nodes from replayable traces and corroborate structural nodes (a bounded confidence boost), never veto (EV-6/RT-5). The gate measures **ten** dimensions (adding `journey_corroboration`) with an **8-mutation** self-test (adding `corrupted-trace-ref` and `observed-unbacked`), and `reproducibility` now applies the RT-8 split standard: static artefacts stay byte-identical while runtime artefacts must be written pre-normalized (no timestamps, session ids, cookie/date values) so re-runs compare byte-equal, screenshots excepted (hash-integrity only). The `taskdesk-legacy` run walked a `login → task list → task detail` journey as `operator1` against a disposable database copy under `.work/semantic-loop/runtime/`, instantiating `sem:flow:login-task-review` (status `accepted`, grounded in three replayable traces) and corroborating seven statically derived Entry/Action/View nodes. `examples/taskdesk-legacy-run/` was regenerated: all ten scores 10, self-test 8/8; the source tree — including the committed SQLite file — was byte-identical before and after (verified by hash), and a repeat run reproduced the static artefacts and normalized traces byte-for-byte.
